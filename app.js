...
const viewProfileBtn = document.getElementById('view-profile-btn'); // Dropdown Identity
            ...
            data.posts.reverse().forEach(post => {
                ...
                const newPostHTML = `
                    <div class="post-card" data-id="${post.id}">
                        <div class="vote-col">
                            <button class="vote-btn up-btn ${upDisabled}"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor"><polyline points="18 15 12 9 6 15"></polyline></svg></button>
                            <span class="vote-score">${post.score || 1}</span>
                            <button class="vote-btn drop-btn ${dropDisabled}"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor"><polyline points="6 9 12 15 18 9"></polyline></svg></button>
                        </div>
                        <div class="post-body">
                            <div class="post-meta">
...
closeButtons.forEach(btn => { 
    btn.addEventListener('click', () => { 
        ...
        if(draftsModal) draftsModal.classList.remove('open');
...
publishBtn.addEventListener('click', async () => {
    ...
    try {
        ...
        if (data.success) {
            titleInput.value = ''; editorContent.innerHTML = '';
            titleCounter.innerText = '0/300'; realFileInput.value = '';
...
async function loadComments(postId) {
    ...
    try {
        ...
        commentsList.innerHTML = '<div class="auth-error">Failed to load comments.</div>';
    }
}
...
if(mainFeed) {
    mainFeed.addEventListener('click', async (e) => {
        ...
        if (e.target.classList.contains('poll-option') || e.target.tagName === 'A') return;
...
if(customUploadBtn) {
    ...
    realFileInput.addEventListener('change', () => {
        if (realFileInput.files.length > 0) {
            ...
            uploadText.style.color = 'var(--accent-bright)';
...
document.body.addEventListener('click', (e) => {
    if (e.target.classList.contains('poll-option')) {
        ...
        container.querySelectorAll('.poll-option').forEach(opt => {
            ...
            opt.style.borderColor = 'var(--border-2)';
...
if (saveDraftBtn) {
    saveDraftBtn.addEventListener('click', (e) => {
        ...
        const newDraft = {
            id: Date.now(), title: title || "Untitled Draft", content: content,
            ...
            const draftHTML = `
                <div class="draft-item">
                    ...
                    <div style="display: flex; gap: 10px;">
                        <button class="btn-ghost load-draft-btn" data-id="${draft.id}" style="padding: 4px 12px;">Load</button>
                        <button class="btn-ghost delete-draft-btn" data-id="${draft.id}" style="padding: 4px 12px; color: var(--red);">Delete</button>
                    </div>
                </div>`;
...
if (draftsListContainer) {
    draftsListContainer.addEventListener('click', (e) => {
        ...
        if (deleteBtn) {
            ...
            if (drafts.length === 0) draftsModal.classList.remove('open');
        }
    });
}
...
async function loadPendingUsers() {
    ...
    pendingUsersList.innerHTML = '<div class="no-comments"><p>Loading...</p></div>';
    ...
    try {
        ...
        if (data.success) {
            if (data.users.length === 0) {
                pendingUsersList.innerHTML = '<div class="no-comments"><p style="color: var(--green);">✅ All caught up! No pending applications.</p></div>';
                return;
            }
            ...
            data.users.forEach(user => {
                ...
                const userCard = `
                    <div class="draft-item" style="flex-direction: column; align-items: stretch; gap: 10px;">
                        <div>
                            <h4>${user.username}</h4>
                            <span style="display:block; margin-bottom: 2px;">${user.email}</span>
                            <span>${user.course} | ${user.year} | Applied: ${dateApplied}</span>
                        </div>
                        <div style="display: flex; gap: 10px;">
                            <button class="btn-submit approve-btn" data-email="${user.email}" style="flex:1; padding: 6px; background: var(--green);">Approve</button>
                            <button class="btn-ghost reject-btn" data-email="${user.email}" style="flex:1; padding: 6px; color: var(--red); border-color: var(--red);">Reject</button>
                        </div>
                    </div>
                `;
            ...
            pendingUsersList.innerHTML = `<div class="auth-error">${data.message}</div>`;
        }
...
if (pendingUsersList) {
    pendingUsersList.addEventListener('click', async (e) => {
        ...
        try {
            ...
            if (data.success) { loadPendingUsers(); } 
            else { alert("Failed: " + data.message); btn.innerText = originalText; btn.disabled = false; }
